...
async function authorize(teamId,p,roles){const m=p&&await membership(teamId,p.userId);return m&&(!roles||roles.includes(m.role))?m:null}
async function deleteEntitySafe(c,pk,rk){try{await c.deleteEntity(pk,rk)}catch(e){if(e.statusCode!==404)throw e}}
async function deleteByFilter(tableName,filter){const c=table(tableName);await ensure(c);let count=0;for await(const x of c.listEntities({queryOptions:{filter}})){await deleteEntitySafe(c,x.partitionKey,x.rowKey);count++}return count}
