...
// Fase 2: opstellingen, 5-1 rotaties en automatische servicewissels
...
async function getLineup(teamId,matchId,setNumber){let c=tableClient('VTMLineups');await ensureTable(c);try{return{client:c,item:await c.getEntity(teamId,lineupKey(matchId,setNumber))}}catch(e){if(e.statusCode===404)return{client:c,item:null};throw e}}
function publicLineup(x){if(!x)return null;let players={};for(const r of LINEUP_ROLES)players[r]={playerId:x[`${r}Id`]||'',name:x[`${r}Name`]||''};return{lineupId:x.lineupId,matchId:x.matchId,setNumber:Number(x.setNumber),players,startRole:x.startRole,initialServer:x.initialServer,serving:x.serving,rotation:Number(x.rotation||1),updatedAt:x.updatedAt||''}}
async function phase2Status(teamId,matchId){let st=await liveStatus(teamId,matchId),ln=st.currentSet?await getLineup(teamId,matchId,st.currentSet.setNumber):{item:null};st.lineup=publicLineup(ln.item);return st}
async function rotationLog(matchId,teamId,setNumber,pointId,beforeServing,afterServing,beforeRotation,afterRotation,reason,p){let c=tableClient('VTMRotations');await ensureTable(c),id=`${Date.now()}_${createId()}`;await c.createEntity({partitionKey:matchId,rowKey:id,rotationId:id,teamId,matchId,setNumber,pointId,beforeServing,afterServing,beforeRotation,afterRotation,reason,createdAt:new Date().toISOString(),createdBy:p.userId});return id}
liveRoute('lineupGet','lineup',['GET'],async req=>{let p=getPrincipal(req);if(!p)return response(401,{error:'Niet ingelogd'});let teamId=req.query.get('teamId'),matchId=req.query.get('matchId'),setNumber=Number(req.query.get('setNumber')),a=await liveAccess(teamId,matchId,p);if(a.error)return a.error;return response(200,{lineup:publicLineup((await getLineup(teamId,matchId,setNumber)).item)})});
liveRoute('lineupSave','lineup/save',['POST'],async req=>{let p=getPrincipal(req);if(!p)return response(401,{error:'Niet ingelogd'});let b=await req.json(),a=await liveAccess(b.teamId,b.matchId,p,['Owner','Coach']);if(a.error)return a.error;if(!LINEUP_ROLES.includes(b.startRole)||!['team','opponent'].includes(b.initialServer))return response(400,{error:'Ongeldige startopstelling'});let ids=LINEUP_ROLES.map(r=>clean(b.players?.[r]?.playerId));if(ids.some(x=>!x)||new Set(ids).size!==6)return response(400,{error:'Selecteer zes verschillende spelers'});let players=tableClient('VTMPlayers');await ensureTable(players),entity={partitionKey:b.teamId,rowKey:lineupKey(b.matchId,b.setNumber),lineupId:lineupKey(b.matchId,b.setNumber),matchId:b.matchId,setNumber:Number(b.setNumber),startRole:b.startRole,initialServer:b.initialServer,serving:b.initialServer,rotation:LINEUP_SEQUENCE.indexOf(b.startRole)+1,updatedAt:new Date().toISOString(),updatedBy:p.userId};for(const r of LINEUP_ROLES){let id=clean(b.players[r].playerId),pl;try{pl=await players.getEntity(b.teamId,id)}catch(e){return response(400,{error:`Speler voor ${r} niet gevonden`})}entity[`${r}Id`]=id;entity[`${r}Name`]=clean(pl.name)}let c=tableClient('VTMLineups');await ensureTable(c);await c.upsertEntity(entity,'Replace');return response(200,await phase2Status(b.teamId,b.matchId))});
liveRoute('phase2Status','phase2/status',['GET'],async req=>{let p=getPrincipal(req);if(!p)return response(401,{error:'Niet ingelogd'});let teamId=req.query.get('teamId'),matchId=req.query.get('matchId'),a=await liveAccess(teamId,matchId,p);if(a.error)return a.error;return response(200,await phase2Status(teamId,matchId))});
liveRoute('phase2Point','phase2/point',['POST'],async req=>{let p=getPrincipal(req);if(!p)return response(401,{error:'Niet ingelogd'});let b=await req.json();if(!['team','opponent'].includes(b.winner))return response(400,{error:'Ongeldige winnaar'});let a=await liveAccess(b.teamId,b.matchId,p,['Owner','Coach']);if(a.error)return a.error;if(a.match.status!=='Live')return response(409,{error:'Deze wedstrijd is niet actief'});let sd=await liveSets(b.teamId,b.matchId),s=sd.items.find(x=>!x.completed);if(!s)return response(409,{error:'Start eerst de volgende set'});let ld=await getLineup(b.teamId,b.matchId,s.setNumber),ln=ld.item;if(!ln)return response(409,{error:'Sla eerst de opstelling voor deze set op'});let beforeServing=ln.serving,beforeRotation=Number(ln.rotation||1),changed=b.winner!==beforeServing,afterServing=changed?b.winner:beforeServing,afterRotation=beforeRotation;if(changed&&b.winner==='team')afterRotation=beforeRotation%6+1;ln.serving=afterServing;ln.rotation=afterRotation;ln.updatedAt=new Date().toISOString();await ld.client.updateEntity(ln,'Replace');s.teamScore=Number(s.teamScore||0)+(b.winner==='team'?1:0);s.opponentScore=Number(s.opponentScore||0)+(b.winner==='opponent'?1:0);if(isSetWon(s.teamScore,s.opponentScore,Number(s.setNumber))){s.completed=true;s.winner=s.teamScore>s.opponentScore?'team':'opponent';s.completedAt=new Date().toISOString()}s.updatedAt=new Date().toISOString();await sd.client.updateEntity(s,'Replace');let pc=tableClient('VTMPoints');await ensureTable(pc),pointId=`${Date.now()}_${createId()}`,rotationId='';if(changed)rotationId=await rotationLog(b.matchId,b.teamId,Number(s.setNumber),pointId,beforeServing,afterServing,beforeRotation,afterRotation,b.winner==='team'?'Service overgenomen en doorgedraaid':'Service verloren',p);await pc.createEntity({partitionKey:b.matchId,rowKey:pointId,pointId,teamId:b.teamId,matchId:b.matchId,setNumber:Number(s.setNumber),winner:b.winner,teamScore:s.teamScore,opponentScore:s.opponentScore,beforeServing,afterServing,beforeRotation,afterRotation,rotationId,createdAt:new Date().toISOString(),createdBy:p.userId});return response(200,await phase2Status(b.teamId,b.matchId))});
liveRoute('phase2Undo','phase2/undo',['POST'],async req=>{let p=getPrincipal(req);if(!p)return response(401,{error:'Niet ingelogd'});let b=await req.json(),a=await liveAccess(b.teamId,b.matchId,p,['Owner','Coach']);if(a.error)return a.error;let sd=await liveSets(b.teamId,b.matchId),s=[...sd.items].reverse().find(x=>Number(x.teamScore||0)+Number(x.opponentScore||0)>0);if(!s)return response(409,{error:'Geen punt om terug te draaien'});let pd=await livePoints(b.matchId,s.setNumber),last=pd.items.at(-1);if(!last)return response(409,{error:'Geen punt om terug te draaien'});let ld=await getLineup(b.teamId,b.matchId,s.setNumber);if(ld.item){ld.item.serving=last.beforeServing||ld.item.serving;ld.item.rotation=Number(last.beforeRotation||ld.item.rotation);ld.item.updatedAt=new Date().toISOString();await ld.client.updateEntity(ld.item,'Replace')}if(last.rotationId){let rc=tableClient('VTMRotations');await ensureTable(rc);try{await rc.deleteEntity(b.matchId,last.rotationId)}catch(e){if(e.statusCode!==404)throw e}}await pd.client.deleteEntity(b.matchId,last.rowKey);s.teamScore=Math.max(0,Number(s.teamScore||0)-(last.winner==='team'?1:0));s.opponentScore=Math.max(0,Number(s.opponentScore||0)-(last.winner==='opponent'?1:0));s.completed=false;s.winner='';s.completedAt='';s.updatedAt=new Date().toISOString();await sd.client.updateEntity(s,'Replace');return response(200,await phase2Status(b.teamId,b.matchId))});
liveRoute('phase2Report','report-phase2',['GET'],async req=>{let p=getPrincipal(req),teamId=req.query.get('teamId');if(!p||!await requireMember(teamId,p))return response(403,{error:'Geen toegang'});let result={teamId,generatedAt:new Date().toISOString(),players:[],matches:[],sets:[],points:[],lineups:[],rotations:[],attendance:[],substitutions:[]},teamTables={players:'VTMPlayers',matches:'VTMMatches',sets:'VTMSets',lineups:'VTMLineups',attendance:'VTMAttendance',substitutions:'VTMSubstitutions'};for(const [k,n] of Object.entries(teamTables)){let c=tableClient(n);await ensureTable(c);for await(const x of c.listEntities({queryOptions:{filter:`PartitionKey eq '${teamId}'`}}))result[k].push(x)}for(const [k,n] of Object.entries({points:'VTMPoints',rotations:'VTMRotations'})){let c=tableClient(n);await ensureTable(c);for await(const x of c.listEntities({queryOptions:{filter:`teamId eq '${teamId}'`}}))result[k].push(x)}return response(200,result)});
